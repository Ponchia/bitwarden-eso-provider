...
- Vaultwarden user API key client ID and client secret.
- Vaultwarden user master password.
...
- Provider pod to Vaultwarden HTTPS endpoint.
...
- Vaultwarden credentials must come from a Kubernetes Secret or external
  workload identity mechanism, not command-line args.
- The default deployment must watch or serve only configured namespaces.
- A compromised application namespace must not allow arbitrary Vaultwarden item
...
- Whether a dedicated Vaultwarden user can be constrained tightly enough through
  organizations and collections for multi-namespace use.
